...
export function QuickLinkCard({ item }: { item: CardItem }) {
  const body = (
    <div className="card-motion group h-full overflow-hidden rounded-[10px] border border-charcoal/10 bg-white p-6 shadow-sm hover:-translate-y-1 hover:border-charcoal/25 hover:shadow-lift">
      <div className="flex items-center justify-between gap-3">
        <div className="flex h-11 w-11 items-center justify-center rounded-[10px] bg-forest-800 text-sm font-black text-white">
          {item.label?.slice(0, 2).toUpperCase() ?? item.title.slice(0, 2).toUpperCase()}
        </div>
        <span className="rounded-[25px] bg-gold px-3 py-1 text-xs font-black uppercase tracking-[0.12em] text-charcoal">
          {item.label ?? "Info"}
        </span>
      </div>
  ...
  return item.href ? (
    <Link className="focus-ring block rounded-[10px]" href={item.href}>
      {body}
    </Link>
  ) : (
    body
...
export function ProgramCard({ item }: { item: CardItem }) {
  return (
    <article
      ...
      className="card-motion group overflow-hidden rounded-[10px] border border-charcoal/10 bg-white shadow-sm hover:-translate-y-1 hover:shadow-lift"
    >
      {item.image ? (
        <div className="relative h-48 overflow-hidden">
          <img
            ...
            src={item.image}
          />
          <p className="absolute bottom-4 left-4 rounded-[10px] bg-gold px-3 py-1 text-xs font-black uppercase tracking-[0.12em] text-charcoal">
            Program
          </p>
        </div>
      ) : null}
...
export function AnnouncementCard({ item }: { item: CardItem }) {
  return (
    <article className="card-motion group overflow-hidden rounded-[10px] border border-charcoal/10 bg-white shadow-sm hover:-translate-y-1 hover:shadow-lift">
      {item.image ? (
        <img
...
export function EventCard({ item }: { item: CardItem }) {
  return (
    <article className="card-motion group flex gap-4 overflow-hidden rounded-[10px] border border-charcoal/10 bg-white p-4 shadow-sm hover:-translate-y-1 hover:shadow-lift">
      <div className="flex h-16 w-16 shrink-0 flex-col items-center justify-center rounded-[10px] bg-forest-800 text-center text-white">
...
export function StaffCard({ member }: { member: StaffMember }) {
  return (
    <article className="card-motion group overflow-hidden rounded-[10px] border border-charcoal/10 bg-white shadow-sm hover:-translate-y-1 hover:shadow-lift">
      ...
      <h3 className="mt-3 font-display text-2xl text-charcoal">{member.name}</h3>
