...
export function SiteFooter() {
  return (
    <footer className="green-glow text-cream">
      <div className="container-pad grid gap-10 py-14 lg:grid-cols-[1.2fr_1fr_1fr]">
        <div>
          <div className="flex items-center gap-3">
            <span className="flex h-14 w-14 items-center justify-center rounded-organic bg-cream p-1 shadow-warm">
              <img
          ...
          <div className="hand-card mt-5 flex items-center gap-3 border border-cream/10 bg-cream/8 p-3">
            <img
              alt="Okanese First Nation logo"
              className="h-12 w-12 object-contain"
              src="/images/okanese-first-nation-logo.png"
            />
            <p className="text-xs font-semibold leading-5 text-cream/70">
        ...
        <div>
          ...
          <ul className="mt-4 grid gap-2">
            {navItems.slice(1, 6).map((item) => (
              <li key={item.href}>
                <Link
                  className="focus-ring rounded-md text-sm text-cream/80 hover:text-cream"
                  href={item.href}
                >
                  {item.label}
                </Link>
      ...
      <div className="border-t border-cream/10">
        <div className="container-pad py-5 text-sm text-cream/60">
